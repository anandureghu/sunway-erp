...
import {
  DEPARTMENT_CSV_CANONICAL_FIELDS,
  importDepartmentsCsv,
  previewDepartmentsCsv,
  type DepartmentCsvImportResult,
  type DepartmentCsvPreview,
} from "@/service/departmentService";

const TEMPLATE_HEADERS = ["Department Code", "Department Name (EN)", "Description"];
...
type Props = { companyId: number; onImported: () => void };

export function ImportDepartmentsCsvDialog({ companyId, onImported }: Props) {
  const { permissions } = useAuth();
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [importing, setImporting] = useState(false);
  const [preview, setPreview] = useState<DepartmentCsvPreview | null>(null);
  const [mapping, setMapping] = useState<Record<string, string | null>>({});
  const [result, setResult] = useState<DepartmentCsvImportResult | null>(null);
  ...
  const downloadTemplate = () => {
    const sample = [
      ...
      '"DEPT-001","Operations","Day-to-day operations"',
      '"DEPT-002","Finance","Finance and accounting"',
    ].join("\n");
  ...
  const handleFile = async (selected: File | undefined) => {
    ...
    setResult(null);
    try {
      const res = await previewDepartmentsCsv(companyId, selected);
      setPreview(res);
  ...
  const handleConfirmImport = async () => {
    ...
    setResult(null);
    try {
      const res = await importDepartmentsCsv(companyId, file, mapping);
      setResult(res);
      if (res.created > 0) {
        toast.success(`Imported ${res.created} department(s)`);
        onImported();
      } else if (res.failed === 0 && res.skipped > 0) {
        toast.message(`No new departments — ${res.skipped} skipped (duplicate code)`);
      } else {
          ...
          <div className="space-y-4 px-6 py-5 overflow-y-auto max-h-[75vh]">
            <div className="rounded-lg border border-sky-100 bg-sky-50/80 px-3 py-2.5 text-sm text-sky-950">
              <p className="flex items-start gap-2 font-medium">
                ...
                Upload your department spreadsheet as-is
              </p>
              <ul className="mt-1.5 list-disc space-y-1 pl-6 text-xs text-sky-900/80">
                ...
                <li>Duplicate department codes are skipped</li>
                <li>Department code is auto-generated from name if not provided</li>
              </ul>
            </div>
                  ...
                  {mappingEntries.map(([source, target]) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {DEPARTMENT_CSV_CANONICAL_FIELDS.map((field) => {
