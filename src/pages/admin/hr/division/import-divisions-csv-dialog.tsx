...
import {
  DIVISION_CSV_CANONICAL_FIELDS,
  importDivisionsCsv,
  previewDivisionsCsv,
  type DivisionCsvImportResult,
  type DivisionCsvPreview,
} from "@/service/divisionService";
...
export function ImportDivisionsCsvDialog({ onImported }: Props) {
  const { permissions } = useAuth();
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [importing, setImporting] = useState(false);
  const [preview, setPreview] = useState<DivisionCsvPreview | null>(null);
  const [mapping, setMapping] = useState<Record<string, string | null>>({});
  const [result, setResult] = useState<DivisionCsvImportResult | null>(null);
  ...
  const downloadTemplate = () => {
    const sample = [
      ...
      '"DIV-001","Engineering","Operations","Engineering division"',
      '"DIV-002","Procurement","Finance","Procurement division"',
    ].join("\n");
    const blob = new Blob([sample], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "divisions-import-template.csv";
  ...
  const handleFile = async (selected: File | undefined) => {
    ...
    setResult(null);
    try {
      const res = await previewDivisionsCsv(selected);
      setPreview(res);
  ...
  const handleConfirmImport = async () => {
    ...
    setResult(null);
    try {
      const res = await importDivisionsCsv(file, mapping);
      setResult(res);
      if (res.created > 0) {
        toast.success(`Imported ${res.created} division(s)`);
        onImported();
      } else if (res.failed === 0 && res.skipped > 0) {
        toast.message(`No new divisions — ${res.skipped} skipped (duplicate code)`);
      } else {
          ...
          <div className="space-y-4 px-6 py-5 overflow-y-auto max-h-[75vh]">
            <div className="rounded-lg border border-sky-100 bg-sky-50/80 px-3 py-2.5 text-sm text-sky-950">
              <p className="flex items-start gap-2 font-medium">
                ...
                Upload your division spreadsheet as-is
              </p>
              <ul className="mt-1.5 list-disc space-y-1 pl-6 text-xs text-sky-900/80">
                ...
                <li>Department name is matched by name — must already exist</li>
                <li>Duplicate division codes are skipped</li>
              </ul>
            </div>
                  ...
                  {mappingEntries.map(([source, target]) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {DIVISION_CSV_CANONICAL_FIELDS.map((field) => {
