...
function validateCompanyItem(item: CompanyItem): ValidationErrors {
  ...
  if (
    (item.itemStatus === "RETURNED" || item.itemStatus === "LOST") &&
    !item.returnDate
  ) {
...
export default function CompanyPropertiesForm() {
  ...
  const issueCount = items.filter(
    (item) => item.itemStatus === "LOST" || item.itemStatus === "DAMAGED",
  ).length;
  ...
  const reloadFromBackend = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "CompanyPropertiesForm -> failed to load properties:",
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        console.error(
          "CompanyPropertiesForm -> delete failed:",
          err?.response?.data ?? err,
        );
  ...
  const updateItem = useCallback(
    (id: string, changes: Partial<CompanyItem>) => {
      setItems((current) =>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          Please provide accurate information about the company
                          property. This information is used for asset tracking
                    ...
                    <FormRow columns={2}>
                      ...
                      >
                        <select
                          ...
                          onChange={(e) =>
                            updateItem(item.id, {
                              itemStatus:
                                (e.target.value as CompanyItem["itemStatus"]) ||
                                "",
                            })
                          }
                        >
                      ...
                      >
                        <Input
                          ...
                          onChange={(e) =>
                            updateItem(item.id, { dateGiven: e.target.value })
                          }
                          className="rounded-lg border-slate-300"
                        />
                      </FormField>
                    ...
                    <FormRow columns={1}>
                      ...
                      >
                        <Input
                          ...
                          onChange={(e) =>
                            updateItem(item.id, { returnDate: e.target.value })
                          }
                          className="rounded-lg border-slate-300"
                          disabled={item.itemStatus === "ASSIGNED"}
                        />
                        <p className="text-xs text-slate-500">
                          Required for returned/lost items
                        </p>
                      </FormField>
                    </FormRow>
                  </div>
                    ...
                    <div className="relative">
                      <div className="pr-52">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          {item.itemStatus && (
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColor(item.itemStatus)}`}
                            >
                              {item.itemStatus}
                            </span>
                          )}
                  ...
                  {viewingId === item.id && (
                    <div className="space-y-6">
                      <div className="flex items-center justify-between mb-6">
                        ...
                        {item.itemStatus && (
                          <span
                            className={`px-4 py-2 rounded-full text-sm font-semibold border ${getStatusColor(item.itemStatus)}`}
                          >
                            {item.itemStatus}
                          </span>
                        )}
        ...
        {items.length === 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-dashed border-slate-200 p-16 text-center">
            ...
            <p className="text-slate-600">
              Use the Add Property button in the header to create your first
              company property
            </p>
          </div>
        )}
