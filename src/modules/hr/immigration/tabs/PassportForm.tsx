...
export default function PassportForm(): ReactElement {
  ...
  const handleUploadDocument = async (file: File) => {
    ...
    setUploading(true);
    try {
      const res = await immigrationService.uploadPassportDocument(empId, file);
      setDocumentUrl(res?.documentUrl ?? null);
      toast.success("Passport document uploaded");
    } catch (err: any) {
      toast.error(
  ...
  const calculateValidity = () => {
    ...
    const totalDays = Math.floor(
      (expire.getTime() - issue.getTime()) / (1000 * 60 * 60 * 24),
    );
        ...
        actions={
          <>
            {validityConfig && ValidityIcon && (
              <div
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${validityConfig.badgeBg} text-white shadow-lg`}
              >
        ...
        >
          ...
          <div className="relative p-6">
            <div className="flex items-start gap-4">
              ...
              >
                <ValidityIcon
                  className={`h-6 w-6 ${validityConfig.iconColor}`}
                  strokeWidth={2}
                />
              </div>
              <div className="flex-1">
                <h3 className="text-lg font-bold text-slate-900 mb-1">
                  {validityConfig.title}
                </h3>
                <p className={`text-sm ${validityConfig.textColor} mb-3`}>
                  {validity?.status === "valid" &&
                    `Your passport is valid for ${Math.floor(validity.days / 30)} months (${validity.days} days)`}
                  {validity?.status === "expiring" &&
                    `Your passport expires in ${validity.days} days. Consider renewing soon as many countries require 6 months validity.`}
                  {validity?.status === "expired" &&
                    `This passport expired ${Math.abs(validity.days)} days ago. Immediate renewal required for international travel.`}
                </p>
                  ...
                  {validity && (
                    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/60 backdrop-blur-sm rounded-lg text-xs font-medium text-slate-700">
                      ...
                      {validity.days > 0
                        ...
                        : `${Math.abs(validity.days)} days overdue`}
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 border-b border-slate-100 p-5">
              ...
              <div>
                <h3 className="font-bold text-slate-800">Passport Scan</h3>
                <p className="text-xs text-slate-500">
                  Attach a copy of the passport (PDF or image)
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3 p-5">
              {!saved.passportNo ? (
                <p className="text-sm text-slate-500">
                  Save the passport details first, then attach a scan.
                </p>
              ) : (
                <>
                  {documentUrl && (
...
function SummaryItem({
  ...
  return (
    <div className="space-y-1">
      ...
      <div
        className={`text-sm font-semibold ${mono ? "font-mono tracking-wider" : ""} ${highlight ? "text-amber-400" : "text-white"}`}
      >
        {value}
      </div>
    </div>
