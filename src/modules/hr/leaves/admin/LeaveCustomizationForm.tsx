...
export default function LeaveCustomizationForm() {
  ...
  // ── HR policy state (company-level: leave accrual + retirement compensation)
  // These gate the new rules added in the backend. If the toggle is off, the
  // leave matrix below behaves the same as before.
  ...
  // Baseline used by the dirty check. Initialized to the same default so the
  // Save button still works even if the GET fails (e.g. the backend hasn't
  // been restarted with the new endpoint yet).
  const [savedHrPolicies, setSavedHrPolicies] = useState<HrPoliciesPayload>(
    DEFAULT_HR_POLICIES,
  );
  ...
  return (
    <div className="space-y-5 p-0 bg-slate-50/60 min-h-screen">
      ...
      {/* ── HR policy card: company-level accrual + retirement ── */}
