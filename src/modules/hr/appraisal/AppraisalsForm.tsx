...
export default function AppraisalsForm() {
  ...
  const [appraisalYear, setAppraisalYear] = useState<number>(
  ...
  const handleForceDelete = async (appId?: number | string) => {
    ...
    try {
      if (appId && Number(appId))
        await appraisalService.forceDelete(empId, Number(appId));
      setItems((cur) =>
        cur.filter((it) => String(it._localId ?? it.id) !== String(appId)),
      );
  ...
  const handleUnlock = async (appId?: number | string) => {
    ...
    try {
      if (appId && Number(appId))
        await appraisalService.unlock(empId, Number(appId));
  ...
  return (
    <div className="space-y-6 rounded-xl">
      ...
      <SecondaryPageHeader
        ...
        description="Monthly performance evaluations"
      ...
      {(() => {
        const yearItems = items.filter(
          (it) => String(it.year) === String(appraisalYear),
        );
        const draftCount = yearItems.filter(
          (it) => it.status === "DRAFT" || it.status === "SELF_SUBMITTED",
        ).length;
        const reviewedCount = yearItems.filter(
          (it) => it.status === "MANAGER_REVIEWED",
        ).length;
        const lockedCount = yearItems.filter(
          (it) => it.status === "LOCKED",
        ).length;
        const scored = yearItems.filter((it) => it.overallScore != null);
        const avgScore = scored.length
          ? (
              scored.reduce((sum, it) => sum + (it.overallScore || 0), 0) /
              scored.length
            ).toFixed(2)
          : "—";
        return (
              ...
              <div key={key} className="border border-slate-200 rounded-xl p-5">
                {editing ? (
                  <AppraisalEditor
                    ...
                    value={it as any}
                    onChange={(patch) =>
                      updateItem(it._localId ?? it.id, patch as any)
                    }
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span
                          className={`text-xs font-semibold px-2 py-1 rounded-full ${getStatusBadge(it.status)}`}
                        >
                          {it.status || "—"}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2 ml-4 flex-wrap justify-end">
                      ...
                      {isLocked && (
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handleForceDelete(it._localId ?? it.id)
                          }
                  ...
                  <div className="space-y-6">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2 items-center flex-wrap justify-end">
                        <span
                          className={`text-xs font-semibold px-3 py-1 rounded-full ${getStatusBadge(it.status)}`}
                        >
                          {it.status}
                        </span>
                        <Button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      ...
                      <InfoCard
                        icon={TrendingUp}
                        label="KPIs"
                        value={String(it.goals?.length || 0)}
                        color="emerald"
                      />
                      {it.overallScore != null && (
                        <InfoCard
                          icon={TrendingUp}
                          label="Overall Score"
                          value={it.overallScore.toFixed(2)}
                          color="violet"
                        />
                      )}
                    </div>
        ...
        {items.length === 0 && (
          <div className="text-center py-16 border-2 border-dashed border-slate-200 rounded-xl mt-4">
            ...
            <p className="text-slate-500 mb-6 text-sm">
              Click "Add Appraisal" to create a monthly appraisal
            </p>
            <Button
              onClick={handleAddClick}
...
function InfoCard({
  ...
  return (
    <div
      className={`p-4 rounded-xl border flex items-center gap-4 ${colors.container}`}
    >
