...
import {
  TRANSACTION_CSV_CANONICAL_FIELDS,
  importTransactionCsv,
  previewTransactionCsv,
  type FinanceCsvPreview,
  type TransactionCsvImportResult,
...
const FIELD_LABELS: Record<string, string> = {
  ...
  amount: "Amount",
  debitAccountCode: "Debit Account Code",
...
export function ImportTransactionsCsvDialog({ onImported }: Props) {
  ...
  const downloadTemplate = () => {
    const sample = [
      ...
      '"01/09/2024","JOURNAL","45000.00","1100","2100","Rent payment Sep 2024","REF-001"',
      '"15/09/2024","PAYMENT","12500.00","5000","1000","Salary advance","REF-002"',
    ].join("\n");
  ...
  const handleFile = async (selected: File | undefined) => {
    ...
    setResult(null);
    try {
      const res = await previewTransactionCsv(selected);
      setPreview(res);
  ...
  const handleConfirmImport = async () => {
    ...
    setResult(null);
    try {
      const res = await importTransactionCsv(file, mapping);
      setResult(res);
      if (res.created > 0) {
        toast.success(`${res.created} transaction(s) imported`);
        onImported();
      } else if (res.failed === 0 && res.skipped > 0) {
        toast.message(`No transactions created — ${res.skipped} skipped`);
      } else {
        ...
        >
          <div className="bg-gradient-to-r from-slate-800 to-slate-700 flex items-center justify-between px-6 py-4">
            <div className="flex items-center gap-3.5">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border-2 border-white/20 bg-violet-100 text-violet-700">
          ...
          <div className="space-y-4 px-6 py-5 overflow-y-auto max-h-[75vh]">
            ...
            <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-900">
              <p className="flex items-start gap-2 font-semibold">
                ...
                Make sure accounts exist before uploading
              </p>
              <ul className="mt-1.5 list-disc space-y-1 pl-6 text-xs text-amber-800">
                <li>
                  <strong>Chart of Accounts</strong> — debit and credit accounts must already exist; they are matched by account code
                </li>
                <li>
                  Rows missing both debit and credit account codes, or an amount, are skipped automatically
                </li>
              </ul>
            </div>
            ...
            <div className="rounded-lg border border-sky-100 bg-sky-50/80 px-3 py-2.5 text-sm text-sky-950">
              <p className="flex items-start gap-2 font-medium">
                ...
                Upload your transactions spreadsheet as-is
              </p>
              <ul className="mt-1.5 list-disc space-y-1 pl-6 text-xs text-sky-900/80">
                ...
                <li>
                  Transaction types: JOURNAL · PAYMENT · RECEIPT · TRANSFER
                </li>
                <li>Each row creates a new transaction — duplicates are not merged</li>
              </ul>
            </div>
              ...
              <div className="rounded-lg border bg-slate-50 p-3 text-sm space-y-3">
                  ...
                  {mappingEntries.map(([source, target]) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {TRANSACTION_CSV_CANONICAL_FIELDS.map((field) => {
                          const taken =
                ...
                {importing ? (
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span>
                      Importing {preview?.dataRowCount ?? "…"} transactions…
                    </span>
                  </div>
                ) : (
                  <Button
