      ...
      <table className="w-full text-sm">
              ...
              {hg.headers.map(header => {
                ...
                return (
                  <th key={header.id} className="px-4 py-3 text-left select-none">
                    {canSort ? (
                      <button className="inline-flex items-center gap-1 font-medium hover:underline" onClick={header.column.getToggleSortingHandler()}>
                        {flexRender(header.column.columnDef.header, header.getContext())}
                        <span className="text-xs text-gray-500">{sorted === "asc" ? "▲" : sorted === "desc" ? "▼" : ""}</span>
                      </button>
                    ) : (
        ...
        <tbody>
          {table.getRowModel().rows.map((row, idx) => (
            <tr key={row.id} onClick={() => onRowClick?.(row.original)} className={`border-t cursor-pointer transition-colors ${idx % 2 ? "bg-gray-50/60" : "bg-white"} hover:bg-gray-100`}>
          ...
          {table.getRowModel().rows.length === 0 && (
            <tr>
              <td className="px-4 py-6 text-center text-gray-500" colSpan={columns.length}>No rows</td>
            </tr>
          )}
        </tbody>
      </table>
